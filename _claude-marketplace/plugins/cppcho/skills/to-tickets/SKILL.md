...
Write one file per approved ticket under `.scratch/<feature-slug>/issues/<NN>-<slug>.md`, numbered from `01` in dependency order (blockers first). Each file's "Blocked by" lists the numbers/titles it depends on. Use the template below — one ticket per file, never a single combined file.

Keep each ticket to the template's four parts. A ticket carries the behaviour and its acceptance criteria; leave out restated context, rationale sections, and implementation notes.
