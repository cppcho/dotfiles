...
```
pc-exchange
  ● 02 Grant promo credits on development [S] — stack on feat/pc-spend
  ● 03 Count exchanged credits in the monthly quota [S]
  ◐ 04 Exchange promo credits on an owned wallet — 3/5 · feat/pc-exchange #1234
  ○ 08 parked on the dev database · 4 more waiting on tickets above

herdr-auth
  ● 01 Log in with a magic link [M]

Pick: pc-exchange 02 — smallest slice, continues the chain in flight.
```
...
- Done tickets don't get rows, and a fully-done set doesn't get a block — mention it only in passing (`pc-exchange done`) or not at all. Outstanding work is the subject.
