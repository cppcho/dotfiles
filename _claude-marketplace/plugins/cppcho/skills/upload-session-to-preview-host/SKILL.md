---
name: upload-session-to-preview-host
description: Render a Claude Code session transcript to a shareable HTML site and publish it to preview-host.internal. Use when the user wants to share, publish, or get a link for a Claude Code session/transcript/conversation (this one or a past one).
---

# Upload a Claude Code session to preview-host
...
then publishes it to `preview-host.internal` so it has a shareable link.
...
uses the preview-host `upload_site` / `add_site_version` tools.

## Before anything: privacy check

preview-host sites are **permanent** and visible to **anyone with a company SSO login
...
```bash
TOOL="$CLAUDE_PLUGIN_ROOT/skills/upload-session-to-preview-host/session-tool.sh"
```
...
3. **Render** to a static site. Omit the argument for the current session, or pass
   ...
   `page-NNN.html`, fully self-contained, tool-free). Note that size — preview-host caps sites at
...
4. **Publish** with the preview-host `upload_site` tool, pointing at that directory.
   Suggest a readable `name` such as `cc-session-<short id>` or a few words from the
   first prompt — preview-host derives the real slug itself, so report the URL that
...
- preview-host auth uses local Google credentials; on an IAP 401/403 the user runs
