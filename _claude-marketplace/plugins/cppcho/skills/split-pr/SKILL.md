...
Under the table, only what the user needs to decide: why a boundary falls where it does when it is not obvious, any slice that breaks the size guidance and why, deviations from the WIP branch (step 2), each supporting change and the slice that first needs it, preconditions (deploy config, an unreleased dependency), and the WIP files deliberately left out because they belong to another feature. Then ask to proceed. Approval of the plan covers building, pushing the branches and opening the PRs.
...
2. **Files in a reduced state** — write the trimmed version by hand, then audit what the removed code was holding up. The WIP version's other lines were written to fit the whole file, and some of them only make sense beside the part you cut:
   ...
   A file copied at its final state (item 1) can carry the same leftovers when it describes code in a later slice; apply the comment check to it too.
...
6. **Gate** — build, lint and the tests for what changed, using the repo's own commands (README, Makefile, `package.json` scripts, agent rules). A slice that does not build on its own is not self-contained, whatever its diff looks like. Report plainly what you could not run locally and why.
   Then run the gate the other way: revert each file outside the slice's own code and tests — config, test setup, env files, shared helpers — to the base, one at a time, and re-run. A file whose revert leaves the gate green is not this slice's; move it to the slice that needs it. The env or config a file reads directly at runtime is the exception: it stays with its reader even though no test fails without it.
